...
import { removetvDetail } from '../store/reducers/TVSlice';
...
function TVdetail() {
  ...
  <div className='flex w-full   hover:bg-zinc-700 opacity-85 duration-300 rounded-lg p-5 gap-5' >
    ...
    <img className='w-full h-full rounded shadow-[8px_20px_38px_1px_rgba(0,0,0,.5)]  object-cover'    src={`https://image.tmdb.org/t/p/original/${info.detail.poster_path || info.detail.   backdrop_path}`} alt="" />
            ...
            <div className='w-full h-full '  >
                ...
                <div className='ml-7 text-white border p-5 rounded-xl ' >
                             ...
                             <h1 className='text-5xl  font-black' >
                                 ...
                                 <small className='text-2xl ml-3 font-semibold text-zinc-300' >({info.detail.first_air_date.split("-")[0]})</small>
                             </h1>
                            ...
                            {/* detail things */}
                              <div className='flex items-center mt-5 gap-3 ' >
                               ...
                               <h1>{info.detail.genres.map((g,index)=>g.name).join(',')}</h1>
        ...
        {/* tv translations */}
                           <h1 className='text-2xl font-semibold mt-3 mb-1' >tv Trenalated</h1>
  ...
  <div className='flex gap-3 w-[80vh] mt-3 items-center ' >
    ...
    { info.watchProviders && info.watchProviders.buy && info.watchProviders.buy.map((item,index)=>(
     ...
     {/* part:4  Seasons */}
     <hr className='mt-4' />
      <h1 className=' w-[65vh] mx-auto text-2xl mb-2 text-center mt-7 font-semibold text-white  border-b-2   ' >Seasons</h1>
      
      <div className='flex gap-3' >{info.detail.seasons.length > 0 ? info.detail.seasons.map((item,index)=>(
        <div key={index} className='w-[28vh] h-[43vh] rounded-md overflow-hidden flex  ' >

        <div  className='w-full h-full flex flex-col' >   
        <img className='w-full h-[85%] rounded-md shadow-[8px_20px_38px_1px_rgba(0,0,0,.5)]  object-cover top-[10%]' src={`https://image.tmdb.org/t/p/original/${item.poster_path || item.backdrop_path || item.profile_path }`} alt="" />
        
        <h1 className='my-3 ml-3 font-semibold text-zinc-300' >
                       {
                           item.name 
                       }
                   </h1>
      ...
      )):<h1 className='text-3xl text-white font-semibold ' >Not available</h1>}</div>
